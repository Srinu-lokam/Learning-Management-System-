...
<html lang="en">
<head>
    <title>Upload Important Topics</title>
...
<body>
    <div class="container">
        <h2>Upload Important Topics for Module</h2>
        <!-- Upload Form -->
        <form action="uploadimportanttopics" method="post" enctype="multipart/form-data">
            <%
            ...
            <div class="form-group">
                <label for="pdf">Upload Important Topics PDF:</label>
                <input type="file" id="pdf" name="pdf" accept="application/pdf" required>
            </div>
            <div class="upload-btn">
                <button type="submit">Upload Important Topics</button>
            </div>
        </form>
        
        <hr>
        <h3>Uploaded Important Topics</h3>
        
        <!-- Table Displaying Modules with Important Topics -->
                ...
                <tbody>
                    <%
                        PreparedStatement psModules = conn.prepareStatement(
                            "SELECT c.year, c.semester, c.regulation, c.subject, m.module_number, m.important_topics " +
                        ...
                        while (rsModules.next()) {
                            ...
                            String topics = rsModules.getString("important_topics");
                    %>
                    <tr>
                        <td><%= yr %></td>
                        <td><%= sem %></td>
                        ...
                        <td><%= subj %></td>
                        <td><%= modNum %></td>
                        <td><%= (topics != null && !topics.isEmpty()) ? "Uploaded" : "Not Uploaded" %></td>
                        <td>
                            <div class="btn-group">
                                <button class="btn btn-info" onclick="viewTopics('<%= subj %>', '<%= modNum %>')">View</button>
                                <button class="btn btn-danger" onclick="deleteTopics('<%= subj %>', '<%= modNum %>')">Delete</button>
                            </div>
                        </td>
                    </tr>
                    <% } %>
    ...
    <script>
      function viewTopics(subject, moduleNumber) {
          window.location.href = "viewimportanttopics?subject=" + encodeURIComponent(subject) + "&module_number=" + moduleNumber;
      }
      function deleteTopics(subject, moduleNumber) {
          if(confirm("Are you sure you want to delete these important topics?")){
              window.location.href = "deleteimportanttopics?subject=" + encodeURIComponent(subject) + "&module_number=" + moduleNumber;
          }
      }
    </script>
</body>
